...
public class Animator {
	...
	public TextureRegion getRegionPlayer(String key) {
		return textureRegionsPlayer.get(key);
	}

	private static void load() {
		walkSheet = new Texture(Gdx.files.internal(texture));

		TextureRegion tmpPlayer[][] = TextureRegion.split(walkSheet, walkSheet.getWidth() / FRAME_COLS,
				walkSheet.getHeight() / FRAME_ROWS - 1);
		for (int y = 0; y < FRAME_ROWS - 1; y++)
			for (int x = 0; x < FRAME_COLS; x++)
				textureRegionsPlayer.put("player" + y + "_" + x, tmpPlayer[y][x]);
